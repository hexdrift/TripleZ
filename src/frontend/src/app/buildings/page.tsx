...
function usePageFilter() {
  ...
  return { type: "building" as const, value: "", label: "" };
}
...
function BuildingContent() {
  ...
  const baseRooms = useMemo(() => {
    switch (filter.type) {
      case "building": return rooms.filter((r) => r.building_name === filter.value);
      case "gender": return rooms.filter((r) => r.gender === filter.value);
      case "rank": return rooms.filter((r) => r.room_rank === filter.value);
      case "department": return rooms.filter((r) => r.departments.includes(filter.value));
    }
  ...
  const filteredRooms = useMemo(() => {
    ...
    return result;
  }, [baseRooms, filters, sortKey, sortDir]);
  ...
  if (loading) {
    return (
      <div className="surface-card p-10 flex items-center justify-center gap-3">
        ...
        <p className="text-[14px]" style={{ color: "var(--text-3)" }}>טוען נתונים...</p>
      </div>
    );
  }

  // For building view, check building exists
  if (filter.type === "building" && !buildings.find((b) => b.name === filter.value)) {
    return (
      <div>
  ...
  const breadcrumbLabel = filter.type === "building" ? `מבנה ${buildingHe(filter.value)}` : filter.label;
  const exportName = filter.type === "building" ? `מבנה_${buildingHe(filter.value)}_חדרים` : `${filter.label}_חדרים`;

  // Show building column when not filtering by building
  const showBuildingCol = filter.type !== "building";
    ...
    <div>
      ...
      <section className="surface-card p-7 mb-6">
        <div className="flex items-center justify-between">
          <h2 className="section-title">{breadcrumbLabel}</h2>
          <button
            type="button"
            onClick={() => exportToExcel(
              exportName,
              [showBuildingCol ? "מבנה" : null, "חדר", "דרגה", "זירות", "מגדר", "מיטות", "תפוסים", "פנויים", "מצב"].filter(Boolean) as string[],
              filteredRooms.map((r) => [
        ...
        <table className="w-full text-[13px]" style={{ borderCollapse: "collapse" }}>
          <thead>
            <tr style={{ borderBottom: "1px solid var(--border)" }}>
              {showBuildingCol && <th className="px-4 py-3 text-right font-semibold" style={{ color: "var(--text-3)" }}>מבנה</th>}
          ...
          <tbody>
            {filteredRooms.length === 0 ? (
              <tr>
                <td colSpan={showBuildingCol ? 8 : 7} className="px-4 py-10 text-center text-[13px]" style={{ color: "var(--text-3)" }}>
...
function RoomRow({ room, showBuilding, onClick }: { room: Room; showBuilding: boolean; onClick: () => void }) {
  ...
  return (
    ...
    >
      {showBuilding && <td className="px-4 py-3 font-semibold" style={{ color: "var(--text-1)" }}>מבנה {buildingHe(room.building_name)}</td>}
